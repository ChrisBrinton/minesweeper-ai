#!/usr/bin/env python3
"""
Minesweeper AI — Supervised Learning Trainer v3 (V4 Architecture)

Uses the upgraded MinesweeperResNetV4 (96ch, 8 blocks, ~1.37M params).
Data generation uses the v3 iter 9 teacher model (28.4% win rate) for
high-quality self-play data, while training the larger v4 student.

Key changes from v1:
  - V4 architecture (3x more params, deeper, wider)
  - Teacher-student: v3 best model generates data, v4 model trains on it
  - Cross-iteration early stopping (patience across iterations, not just epochs)
  - Starts from scratch (no weight transfer — different architecture)
  - Learning rate warmup for first iteration

Usage:
    nohup python3 -u train_supervised_v3.py > supervised_v3_output.log 2>&1 &
    python3 -u train_supervised_v3.py --device mps --num-iterations 20
"""
...
SUPERVISED_CONFIG = {
    ...
    'batch_size': 64,
    'learning_rate': 1e-4,
    'max_epochs_per_iteration': 75,       # more epochs for larger model
    'patience_epochs': 8,                 # more patience within iteration
    'cross_iteration_patience': 10,       # stop entire run if no improvement for N iters
    'eval_episodes': 2000,                # more eval games for tighter estimates
    'mps_cache_clear_every': 300,         # more frequent cache clearing for bigger model
    'weight_decay': 1e-5,
    'teacher_model': 'models_v3/supervised/expert/best_model.pth',  # v3 iter 9
}
...
def generate_data(model, device, board_cfg, num_games, mps_clear_every=500,
    ...
    Args:
        use_qvalue_mode: If True, interpret model output as Q-values (argmax = best).
                         ...
                         Use True for the pre-trained RL model in iteration 1.
...
def main():
    parser = argparse.ArgumentParser(description='Minesweeper Supervised Trainer v3 (V4 Architecture)')
    ...
    # Paths
    teacher_path = args.model or config['teacher_model']
    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            'models_v4', 'supervised', 'expert')
    os.makedirs(save_dir, exist_ok=True)
    log_path = os.path.join(save_dir, 'training_log.txt')

    # Create V4 student model (starts from scratch — different architecture)
    ...
    with open(log_path, 'a') as log_file:
        header = (f"\n{'='*70}\n"
                  f"SUPERVISED TRAINING v3 (V4 ARCH) START: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        ...
        # ─── Baseline evaluation ──────────────────────────────────────────
        # Evaluate the student (V4) from scratch — should be ~0% since random
        print("Evaluating student baseline (random init)...", flush=True)
        ...
        try:
            for iteration in range(1, config['num_iterations'] + 1):
                ...
                # Use teacher for data gen until student surpasses teacher baseline
                # Teacher generates higher quality data (28.4% win rate)
                use_teacher = (best_win_rate < 0.20)  # switch to student once it's decent
                data_model = teacher_model if use_teacher else model
                data_source = "teacher (V3)" if use_teacher else "student (V4)"
                print(f"Generating data: {config['num_games_per_iteration']} games "
                      f"[{data_source}]...", flush=True)
                gen_start = time.time()
                # Teacher uses P(mine) mode (it was trained supervised too, from iter 2+)
        ...
        summary = (f"\n{'='*70}\n"
                   f"SUPERVISED TRAINING v3 (V4 ARCH) COMPLETE: "
