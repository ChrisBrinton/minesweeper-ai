#!/usr/bin/env python3
"""
Minesweeper AI — Supervised Learning Trainer v1

Instead of RL, train the neural net to predict P(mine) per cell using
ground-truth labels from self-play. The solver handles deterministic moves;
the NN only needs to make good guesses when the solver is stuck.

Algorithm (from ANALYSIS.md §5):
  1. Play N games with hybrid agent (solver + current NN, epsilon=0)
  2. For each guess situation: record board state + hidden cell mask
  3. After game ends: label each hidden cell as mine=1 or safe=0
  4. Train NN with BCE loss on hidden cells only
  5. Evaluate. If improved, save. Repeat.

Usage:
    nohup python3 -u train_supervised_v1.py > supervised_v1_output.log 2>&1 &
    python3 -u train_supervised_v1.py --device cpu
    python3 -u train_supervised_v1.py --num-games 5000 --num-iterations 20
"""
...
def main():
    parser = argparse.ArgumentParser(description='Minesweeper Supervised Trainer v1')
    ...
    # Paths
    pretrained_path = args.model or 'models_v3/hybrid/expert/best_model.pth'
    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            'models_v3', 'supervised', 'expert')
    ...
    with open(log_path, 'a') as log_file:
        header = (f"\n{'='*70}\n"
                  f"SUPERVISED TRAINING v1 START: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        ...
        # ─── Baseline evaluation ──────────────────────────────────────────
        print("Evaluating baseline model (Q-value mode)...", flush=True)
        baseline = evaluate(model, device, board, config['eval_episodes'],
                            use_qvalue_mode=True)
        ...
        try:
            for iteration in range(1, config['num_iterations'] + 1):
                ...
                print(f"Generating data: {config['num_games_per_iteration']} games...",
                      flush=True)
                gen_start = time.time()
                # Iteration 1 uses the pre-trained RL model (Q-values → argmax).
                # After that, the model has been trained with BCE (P(mine) → argmin).
                is_qvalue = (iteration == 1)
                samples, gen_stats = generate_data(
                    model, device, board,
                    config['num_games_per_iteration'],
                    config['mps_cache_clear_every'],
                    use_qvalue_mode=is_qvalue
                )
        ...
        summary = (f"\n{'='*70}\n"
                   f"SUPERVISED TRAINING v1 COMPLETE: "
